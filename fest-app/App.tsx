...
export default function App() {
  ...
  const showPerfumeLanding = Platform.OS === 'web' && process.env.EXPO_PUBLIC_PERFUME_LANDING === '1';

  let content: React.ReactNode;
  if (!fontsLoaded || (!showPerfumeLanding && restoring)) {
    content = (
      <View style={s.fontLoader}>
        <ActivityIndicator size="large" color={theme.colors.primary} />
      </View>
    );
  } else if (showPerfumeLanding) {
    content = <PerfumeLandingScreen />;
